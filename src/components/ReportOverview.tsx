...
export default function ReportOverview({ result }: { result: AnalysisResult }) {
  ...
  const loss = result.financialResults.operatingProfit < 0;
  return (
    <section aria-labelledby="report-takeaway" className="mb-7 overflow-hidden rounded-2xl border border-slate-300 bg-[#edf1ee] text-[#142238]">
      <div className="grid lg:grid-cols-[1.5fr_1fr]">
        ...
        <div className="flex flex-col justify-center border-t border-slate-300 bg-[#dfe7e4] p-6 sm:p-8 lg:border-l lg:border-t-0">
          ...
          <div className={`mt-3 flex items-center gap-3 font-display text-4xl font-semibold ${loss ? 'text-rose-800' : 'text-teal-800'}`}>{formatCurrency(result.financialResults.operatingProfit)}{loss ? <ArrowDownRight className="h-7 w-7" /> : <ArrowUpRight className="h-7 w-7" />}</div>
          <p className="mt-3 text-sm leading-relaxed text-slate-600">Based on the financial inputs for this scenario. Explore the breakdown below.</p>
        </div>
      </div>
    </section>
  );
